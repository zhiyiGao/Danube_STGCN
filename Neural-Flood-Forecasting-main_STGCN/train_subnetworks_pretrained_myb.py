...
PRE_CHECKPOINT_PATH = "G:/dataset/checkpoints/GCNII3"
CHECKPOINT_PATH = "G:/dataset/checkpoints/subnetwork/GCNII29_Pretrain"

for fold_id, (train_years, test_years) in enumerate([(list(range(2015, 2016, 1)), [2016])]):
        ...
        for architecture in ["GCNII"]:
            for edge_orientation in ["downstream"]:
                for adjacency_type in ["all_learned"]:
                    ...
                    # pre_chkpt_name = f"211_{architecture}_{edge_orientation}_{adjacency_type}_2"

                    pre_chkpt_name = f"{architecture}_{edge_orientation}_{'binary'}_2"
                    chkpt = functions.load_checkpoint(f"{PRE_CHECKPOINT_PATH}/{pre_chkpt_name}.run")
                    model = functions.load_model(chkpt, dataset)
